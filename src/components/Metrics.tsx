...
const metrics = [
  {
    ...
    icon: Code2,
    link : "https://codolio.com/profile/Saarthak1234"
  },
  {
    title: "2nd Place",
    description: "Hack Data Hackathon (Healthcare AI category).",
    icon: Trophy,
    link : "https://www.linkedin.com/posts/saarthak-agarwal-sa_hello-everyone-this-past-weekend-i-participated-activity-7305297857564090368-cFpY"
  }
];

export default function Metrics() {
  return (
    <section id="achievements" className="py-12 max-w-5xl mx-auto px-6 w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
        {metrics.map((metric, idx) => {
          ...
          return (
            <Link key={idx} href={metric.link} target="_blank" rel="noopener noreferrer" className="block w-full">
              <motion.div
                initial={{ opacity: 0, scale: 0.95 }}
                whileInView={{ opacity: 1, scale: 1 }}
                viewport={{ once: true, margin: "-50px" }}
                transition={{ delay: idx * 0.1, duration: 0.5, ease: "easeOut" }}
                className="relative h-full flex items-center gap-6 p-8 shadow-ghost bg-black border border-black group-hover:border-[#39FF14] rounded-none transition-all hover:-translate-y-1 group cursor-pointer"
              >
